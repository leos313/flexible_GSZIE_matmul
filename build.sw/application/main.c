...
#define A3_ACCELERATOR_REG_0 (0x000) //the only register of the accelerator is the FLEXIBLE_gsize
...
int main(int argc, char *argv[]) {
    ...
    artico3_kernel_create("matmul", 49152, 3, 1);
    ...
    /* Setting the GSIZE internal of every slot */
    // Writing configuration registers (the number of value of every register to be processed in parallel
    a3data_t rcfg_gsize[N_ACC] = {0, 0, 0, 0};
    a3data_t wcfg_gsize[N_ACC] = {MSIZE_ACC, MSIZE_ACC, MSIZE_ACC, MSIZE_ACC, MSIZE_ACC, MSIZE_ACC, MSIZE_ACC, MSIZE_ACC};
    artico3_kernel_rcfg("matmul", A3_ACCELERATOR_REG_0, rcfg_gsize);
    printf("before writing-----------\n");
    for (i = 0; i < N_ACC; i++) printf("rcfg[%d] = %08x = %d\n", i, rcfg_gsize[i], rcfg_gsize[i]);
    for (i = 0; i < N_ACC; i++) printf("wcfg[%d] = %08x = %d\n", i, wcfg_gsize[i], wcfg_gsize[i]);
    artico3_kernel_wcfg("matmul", A3_ACCELERATOR_REG_0, wcfg_gsize);
    artico3_kernel_rcfg("matmul", A3_ACCELERATOR_REG_0, rcfg_gsize);
    printf("after writing------------\n");
    for (i = 0; i < N_ACC; i++) printf("rcfg[%d] = %08x = %d\n", i, rcfg_gsize[i], rcfg_gsize[i]);

    // Execute kernel
    printf("Executing kernel (and allocating )...\n");
    gettimeofday(&t0, NULL);
